...
We sequence greedily: process blocks left to right and place each item in the
admissible layer of smallest anchor (opening a new layer, anchored at the
current block, when none fits). This costs $O(n \cdot L)$ and, on every
instance we have tested, matches the dual value of
Theorem~\ref{thm:minmax}: $9{,}400$ randomized small instances checked against
a brute-force chromatic number, plus the production parameter sets at
$n \in \{2^{16},2^{18},2^{20}\}$. The dual is itself computable in $O(nb + b^2)$
by a sweep, so optimality can be \emph{certified per instance} rather than
assumed; and if a greedy run ever failed to match it, the LP optimum is
reachable in polynomial time (solve the totally unimodular covering LP, then a
convex bipartite matching per bin~\cite{Glover67}). The bound of
Section~\ref{subsec:budget} is stated for $L^\star$, so it is unaffected by
which of the two the implementation runs.
