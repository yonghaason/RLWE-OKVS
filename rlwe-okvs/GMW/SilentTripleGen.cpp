...
namespace volePSI
{
	void SilentTripleGen::init(u64 n, u64 batchSize, u64 numThreads, Mode mode, block seed)
	{
		...
		auto noise = SdNoiseDistribution::Regular;
		auto sectype = SilentSecType::SemiHonest;

		for (u64 i = 0; i < mSenderOT.size(); i++)
			mSenderOT[i].configure(mNumPer, 2, numThreads, sectype, noise);
		for (u64 i = 0; i < mRecverOT.size(); i++)
			mRecverOT[i].configure(mNumPer, 2, numThreads, sectype, noise);
	}
	...
	Proto SilentTripleGen::generateBaseCors(oc::PRNG& prng, coproto::Socket& chl) {
		if (mMode == Mode::Receiver) {
			...
			oc::SoftSpokenMalOtReceiver mOtExtRecver;
			...
			co_await mOtExtRecver.receive(globChoice, globMsg, prng, chl);

			auto choiceIter = globChoice.data();
			...
			for (u64 i = 0; i < mRecverOT.size(); i++) {
				...
				msgIter += otCount;
				std::span<block> baseA{ &*msgIter, voleCount};
				msgIter += voleCount;
			...
			oc::SoftSpokenMalOtSender mOtExtSender;
			...
			for (u64 i = 0; i < mSenderOT.size(); i++) {
			    ...
			    std::transform(msgIter, msgIter + voleCount, baseB.begin(),
                   [](const std::array<block,2>& a){ return a[0]; });
				msgIter += voleCount;
