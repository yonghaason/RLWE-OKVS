...
void width_test(const oc::CLP& cmd)
{
    ...
    const std::filesystem::path out_dir = std::filesystem::path("tuning") / "okvs_width";
    const std::string ratio_str = [&]() {
        std::ostringstream os;
        os << std::fixed << std::setprecision(2) << m_ratio;
        return os.str();
    }();
    ...
    fname_os << log2ceil(n) << '_'
             ...
             << logp << '_'
             << trials_total;
    const std::string stem = fname_os.str();
    const std::filesystem::path data_path = out_dir / (stem + ".tsv");
